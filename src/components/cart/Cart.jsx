...
const Cart = (props) => {
    ...
    const onAddOverlay = async (obj)=>{
        try{
            ...
            if(findOverlay)
            {
            axios.delete(`https://63805fc08efcfcedac03008d.mockapi.io/cart/${findOverlay.id}`)
            props.setOverlayItems((over) => over.filter(item => item.myId !== obj.myId))
            }
            else{
              const {data} = await axios.post('https://63805fc08efcfcedac03008d.mockapi.io/cart', obj)
    ...
    const onAddFav = async (obj)=>{
        ...
        try{
            const findFavorites = props.favorites.find(objFav => objFav.myId === obj.myId)
             if(findFavorites){
            axios.delete(`https://63805fc08efcfcedac03008d.mockapi.io/favorites/${findFavorites.id}`)
            props.setFavorites((over) => over.filter(item => item.myId !== obj.myId))

        }
        else{
          const {data} = await  axios.post('https://63805fc08efcfcedac03008d.mockapi.io/favorites', obj)
        ...
        <div className={style.cart_section}>
            <div className={style.search}>
                ...
                <div className={style.search_block}>
                    <img src="/images/search.png" alt="поиск"></img>
